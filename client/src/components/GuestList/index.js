    ...
    const saveParty = async (event) => {
        ...
        const { data } = await addParty({
            variables: {
                ...
                person2Id: newMember.memberId
                // person1_id:currentGuest._id,
                // person2_id:newMember.memberId
            }
        })
        ...
        <>
            ...
            <Modal show={groupModal}>
                ...
                <Modal.Body>
                    ...
                    <form>
                        <select onChange={handlePartyChange}>
                            {guests.map(opt => {
                                return (
                                    <option name="newMemberId" value={opt._id}>{opt._id}</option>
                                )
                            })}
                        </select>
                    </form>
                ...
                <Modal.Footer style={{ display: "flex", justifyContent: "center" }}>
                    <Button onClick={saveParty}> Save Changes</Button>
                </Modal.Footer>
            </Modal>
